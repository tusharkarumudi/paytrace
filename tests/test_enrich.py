...
class _Index:
    ...
    SELLERS = {
        "hoang phu linh": [("google.com", "pub-1117393687149626", ""),
                           ("pubmatic.com", "156423", "example.test")],
    }
    SITES = {
        ("google.com", "pub-1117393687149626"): ["snapvn.com", "insget.net"],
...
def test_variants_cover_diacritics_and_initials():
    ...
    v = [x.lower() for x in name_variants("HOÀNG PHÚ LINH")]
    assert "hoàng phú linh" in v          # original preserved, always first
    assert "hoang phu linh" in v          # diacritics folded
    assert any(x.startswith("h. p.") or x.startswith("h p") for x in v)
...
async def test_reverse_sellers_recovers_the_whole_estate():
    ...
    exp = await expand_from_name("HOÀNG PHÚ LINH", index=_Index())

    assert "google.com/pub-1117393687149626" in exp.discovered["seller accounts"]
    assert "pubmatic.com/156423" in exp.discovered["seller accounts"]
    sites = exp.discovered["authorising sites"]
    assert {"snapvn.com", "insget.net", "third-site.example"} <= set(sites)


@pytest.mark.asyncio
async def test_reverse_sellers_matches_on_a_folded_variant():
    """The corpus holds 'hoang phu linh'; the payee record has diacritics."""
    exp = await expand_from_name("HOÀNG PHÚ LINH", index=_Index())
    via = [c for c in exp.claims if c.raw.get("matched_variant")]
    assert via and via[0].raw["matched_variant"].lower() == "hoang phu linh"


@pytest.mark.asyncio
async def test_missing_index_is_reported_as_not_run_not_as_empty():
    exp = await expand_from_name("HOÀNG PHÚ LINH")
...
async def test_document_search_finds_the_name_and_colocated_emails():
    page = ("<html><body><p>Operated by Hoang Phu Linh.</p>"
            "<p>Contact: legal@example.test or dmca@example.test</p></body></html>")
    f = _Fetcher({"https://snapvn.com/terms-of-service": page})
    exp = await expand_from_name("HOÀNG PHÚ LINH", fetcher=f, index=_Index(),
                                 known_domains=["snapvn.com"])
    assert "https://snapvn.com/terms-of-service" in exp.discovered[
...
async def test_document_search_reports_empty_when_the_name_is_absent():
    f = _Fetcher({"https://snapvn.com/terms": "<p>nothing relevant</p>"})
    exp = await expand_from_name("HOÀNG PHÚ LINH", fetcher=f, index=_Index(),
                                 known_domains=["snapvn.com"])
