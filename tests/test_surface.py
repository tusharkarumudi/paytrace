...
def test_wordpress_author_slug_leaks_the_login():
    html = '<a href="/author/hoanglinh/">posts</a>'
    handles = _objs(_harvest({"https://target.example/": html}), IdKind.HANDLE)
    assert "wp:hoanglinh" in handles
...
def test_wp_rest_users_endpoint_yields_login_names():
    users = '[{"slug":"hoanglinh","name":"Hoang Phu Linh"}]'
    claims = _harvest({
        "https://target.example/": "<html></html>",
        "https://target.example/wp-json/wp/v2/users": users})
    assert "wp:hoanglinh" in _objs(claims, IdKind.HANDLE)
    assert "Hoang Phu Linh" in _objs(claims, IdKind.PERSON_NAME)


def test_webfinger_yields_federated_identity():
    wf = '{"subject":"acct:linh@target.example","links":[]}'
    claims = _harvest({
        "https://target.example/": "<html></html>",
        "https://target.example/.well-known/webfinger": wf})
    assert "linh@target.example" in _objs(claims, IdKind.EMAIL)
