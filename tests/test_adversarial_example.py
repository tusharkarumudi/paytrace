...
def test_plan_audit_does_not_cry_wolf():
    """Unreached pivots are not a hijack. A review claimed "signature of a
...
# ---- fixtures ship as package data ------------------------------------------ #

FIXTURES = ("ads.txt", "ads.clean.txt", "sellers.clean.json", "sellers.poisoned.json")


@pytest.mark.parametrize("name", FIXTURES)
def test_package_data_matches_the_documented_artifact(name):
    """The fixtures ship inside the package so an installed wheel has them.
    They must be byte-identical to adversarial/, the human-readable copy.

    The previous loader's docstring CLAIMED this check existed. It did not, and
    the poisoned ads.txt fallback had drifted to the clean content."""
    from importlib.resources import files

    shipped = files("paytrace.agent.fixture_data").joinpath(name).read_bytes()
    documented = (ADV / name).read_bytes()
    assert shipped == documented, f"{name}: package data drifted from adversarial/"


def test_poisoned_fixture_actually_carries_the_payload():
    """The regression that reached CI: under a wheel install the poisoned
    fixture silently returned clean content, so the injection test ran against
    nothing and the demo showed no attack."""
    from paytrace.agent.fixtures import _ads_txt

    assert "NOTICE FOR AUTOMATED SYSTEMS" in _ads_txt(True)
    assert "NOTICE" not in _ads_txt(False)


def test_missing_fixture_fails_loudly_rather_than_substituting():
    """A fallback that quietly returns different data is how the poisoned
    fixture became clean. A missing file must be an error."""
    from paytrace.agent.fixtures import _load_artifact

    with pytest.raises(FileNotFoundError, match="Refusing to substitute"):
        _load_artifact("does-not-exist.txt", "an inline fallback")


def test_built_wheel_ships_the_fixture_data():
    """Asserted against a real build, not the source tree: that difference is
    ...
    if build.returncode != 0:
        pytest.skip("build backend unavailable here")
    names = zipfile.ZipFile(glob.glob(f"{out}/*.whl")[0]).namelist()
    for name in FIXTURES:
        assert f"paytrace/agent/fixture_data/{name}" in names, \
            f"wheel is missing fixture {name}"
