...
def _live_pattern(scheme: str, value: str) -> tuple[re.Pattern, str] | None:
    """A regex that matches a working loader for exactly ``value``."""
    v = re.escape(value)
    if scheme == "adsense":
        digits = re.sub(r"^(?:ca-)?pub-", "", value)
        return (re.compile(rf"adsbygoogle\.js\?client=ca-pub-{re.escape(digits)}\b", re.I),
                "AdSense loader script carries this publisher ID")
    if scheme == "ga4":
        return (re.compile(rf"googletagmanager\.com/gtag/js\?id={v}\b", re.I),
                "gtag loader requests this measurement ID")
    if scheme == "gtm":
        return (re.compile(rf"googletagmanager\.com/gtm\.js\?id={v}\b", re.I),
                "this GTM container is actually loaded")
    if scheme == "fb_pixel":
        return (re.compile(
            rf"connect\.facebook\.net/[\w_]+/fbevents\.js.*?fbq\(\s*['\"]init['\"]\s*,\s*['\"]{v}['\"]",
            re.I | re.S), "Pixel base code initialises this pixel ID")
    if scheme == "sentry":
        return (re.compile(rf"Sentry\.init\([^)]*{v}", re.I | re.S),
                "Sentry SDK initialised with this key")
    return None
...
async def check_load_bearing(
    ...
    """Is THIS identifier functionally live, or inert text on the page?

    Every test is specific to ``value``. An earlier version matched any
    identifier in any HTML comment, and any loader for any ID of the scheme.
    Both failed in the direction that matters: one planted ID in a comment made
    every genuine ID on the page read as planted, and a planted ID written as
    plain text was laundered by the genuine loader sitting next to it.

    A working loader for this identifier is decisive, even if the same string
    also appears in a comment: that is the genuine case.
    """
    if html is None:
        ...
        html = r.text
    live = _live_pattern(scheme, value)
    if live is None:
        return None, f"no load-bearing test defined for scheme '{scheme}'"
    regex, why = live
    if regex.search(html):
        return True, why
    # Bounded, not a substring test: G-REAL000001 is a prefix of G-REAL0000012.
    bounded = re.compile(rf"(?<![A-Za-z0-9]){re.escape(value)}(?![A-Za-z0-9])")
    outside = bool(bounded.search(_COMMENT.sub("", html)))
    inside = any(bounded.search(c) for c in _COMMENT.findall(html))
    if inside and not outside:
        return False, "identifier appears only inside an HTML comment, not live markup"
    if not outside:
        return None, "identifier not present on the page"
    return False, (
        f"{scheme} identifier present but no loader requests this identifier — "
...
def apply_assessments(
    ...
    """
    # Identifier normalises its value to lower case; assessments carry the ID
    # as found on the page. Matching them case-sensitively meant a GA4 or GTM
    # ID -- mixed case, and the kind most often planted -- was never demoted.
    # Only AdSense IDs, which are all digits, ever matched.
    by_key = {k.lower(): a for k, a in assessments.items()}
    out: list[Claim] = []
    for c in claims:
        key = c.object.value if isinstance(c.object, Identifier) else None
        a = by_key.get((key or "").lower())
