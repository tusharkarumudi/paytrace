...
def _load_artifact(name: str, fallback: str) -> str:
    """Read a committed artifact, falling back to an inline copy.

    The fallback exists only so an installed wheel (which ships `src/`, not
    `adversarial/`) still runs the demo. A mismatch between the two is caught
    by `tests/test_adversarial_example.py`, which asserts they agree.
    """
...
class FixtureIndex:
    ...
    _PORTFOLIO = {
        ...
        ("ga4", "G-K7X2M9QP1L"): [DOMAIN, "mirror-a.example"],
    }
